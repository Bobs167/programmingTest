...
int main(){
	...
	int timeOutCount = 0;//タイムアウトした回数を記録
	int firstErrorFlag = 0;//最初のエラーのフラグ
	int flag = 0;//フラグ処理用変数
	int N; //許容できるタイムアウトの回数
	
	printf("許容できるタイムアウトの回数を入力してください\n");
	scanf("%d\n",&N);
	printf("%d回までタイムアウトを許容\n",N);
	...
	while(fscanf(fp, "%lf,%[^,],%s", &dayTime, adress, p) != EOF) {
	    ...
	    if(flag == 1){
	        ...
	        ,year - errorTime_year
	        ,month - errorTime_month
	        ,day - errorTime_day
	        ,hour - errorTime_hour
	        ,minute - errorTime_minute
	        ,seconds - errorTime_seconds);
